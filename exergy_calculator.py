...
MODEL_DIR = os.path.join(FOLDER, "ANN_models")          # outputs live alongside the other steps
OUT_XLSX = os.path.join(MODEL_DIR, "exergy_results.xlsx")  # v2 removed from filename
...
SECTION_BLOCKS = {
 ...
 "Coal Mill":     [("GASHEATE","work"),("CM-GRIND","work"),("CM-DRYER","heat")],
...
# ============================================================ BLOCK / ENTROPY ==
DUTY_NODE_OVERRIDE = {"KLNLOSS": ["QCALC","QNET","Q"],
                      "CM-GRIND": ["QCALC","QNET","Q"]}   # blocks whose net
...
def main():
    ...
    print("="*72); print("EXERGY CALCULATOR  (reference streams + live heat/work blocks)"); print("="*72)
    ...
    # ---- BASE CASE via WRITE-THEN-SOLVE (Reinit -> write base inputs -> Run2 xN) ----
    # The whole pipeline (campaign samples, PSO validation) reaches every operating
    # point by writing inputs and re-solving. The base case is solved the SAME way
    # here so it lies on the SAME converged branch as every other result -> a single,
    # self-consistent base value everywhere. This is MANDATORY: if the driver cannot
    # be imported or the write-then-solve fails, we STOP rather than silently fall
    # back to the untouched solve (which lands on the OTHER branch and would corrupt
    # the single-branch consistency this pipeline now relies on).
    os.makedirs(MODEL_DIR, exist_ok=True)
    sys.path.insert(0, FOLDER)              # ensure the driver is importable first
    try:
        import exergy_campaign_driver as DRV
    except Exception as e:
        sys.exit("FATAL: cannot import exergy_campaign_driver (%s).\n"
                 "       Run this from the AspenPlus folder so write-then-solve is used.\n"
                 "       (No silent fallback - the base case MUST be on the write-then-solve branch.)"
                 % str(e)[:120])
    print(">> running base case (WRITE-THEN-SOLVE, consistent branch)...")
    base = DRV.base_sample()
    keys = list(DRV.INPUTS.keys())
    DRV.apply_and_run(a, base, keys)        # Reinit -> write 26 inputs -> Run2 x SETTLE_RUNS
    print("   done (write-then-solve, %d settle runs)."%getattr(DRV,"SETTLE_RUNS",2))
    ...
    # ---- FUEL EXERGY on a FIXED (property-based) basis --------------------------
    # The stream chemical-exergy tally (coal_ex above) is re-solve dependent: when
    # operating inputs change and Aspen re-converges, the booked coal chemical
    # exergy drifts even at constant coal MASS, which makes the efficiency ratio
    # move for non-physical reasons. To get an efficiency that reflects only real
    # thermodynamic change, the fuel exergy is evaluated from the coal MASS flow
    # times a fixed specific chemical exergy (Szargut beta x NCV), a property of
    # the coal that does NOT depend on the solve. coal_ex (stream tally) is still
    # reported for reference.
    def coal_mass(stream):
        r=SR.get(stream)
        return r["m"] if (r and r.get("m")) else 0.0
    m_coal = coal_mass("CALCOAL") + coal_mass("KILNCOAL")        # kg/s total firing
    ex_sp  = coal_beta(read_coal_ultimate(a, "CALCOAL"))        # J/kg, fixed property
    fuel_ex_fixed = m_coal*ex_sp                                # W, solve-independent
    # overall rational efficiency on the fixed fuel-exergy basis (primary):
    epsP = clk/(fuel_ex_fixed+elec) if (fuel_ex_fixed+elec)>0 else 0
    # stream-tally efficiency kept for reference / back-compatibility:
    epsP_stream = clk/(coal_ex+elec) if (coal_ex+elec)>0 else 0

    # ---- SPECIFIC IRREVERSIBILITY ----------------------------------------------
    # Primary (per-mass): total plant exergy destruction per kg of clinker
    # produced. tot_IB is in W (= J/s); clinker MASS flow is in kg/s; their ratio
    # is J/kg, reported as MJ/kg clinker (equivalently GJ/tonne clinker). This is
    # the physically intuitive "exergy destroyed per kg of product" measure.
    def stream_mass(s):
        r=SR.get(s); return r["m"] if (r and r.get("m")) else 0.0
    m_clinker = stream_mass("CLINKOUT")                         # kg/s
    spec_IB_MJkg = (tot_IB/m_clinker)/1e6 if m_clinker>0 else float("nan")  # MJ/kg
    # Secondary (dimensionless): destruction per unit clinker EXERGY [MW/MW];
    # kept because the Step-4 PSO objective is defined on this ratio.
    spec_IB = tot_IB/clk if clk>0 else float("nan")
    ...
    print("  Clinker mass flow             = %8.4f kg/s"%m_clinker)
    print("  Specific irreversibility      = %8.4f MJ/kg clinker  (primary, per-mass)"%spec_IB_MJkg)
    print("  Specific irreversibility      = %8.4f MW/MW clinker   (dimensionless, ref)"%spec_IB)
    print("  Coal chemical exergy (stream) = %8.3f MW  (reference)"%(coal_ex/1e6))
    print("  Coal mass firing              = %8.4f kg/s"%m_coal)
    print("  Fuel exergy (fixed basis)     = %8.3f MW  (m_coal x %.2f MJ/kg)"%(fuel_ex_fixed/1e6, ex_sp/1e6))
    print("  Electrical work in            = %8.3f MW"%(elec/1e6))
    print("  Clinker product exergy        = %8.3f MW"%(clk/1e6))
    print("  Overall plant efficiency      = %8.2f %%  (fixed fuel-exergy basis, primary)"%(epsP*100))
    print("  Overall efficiency (stream)   = %8.2f %%  (re-solve dependent, reference)"%(epsP_stream*100))
    if FLAGS:
        print("\n  !! VALIDATION FLAGS:")
    ...
    write_workbook(openpyxl, SR, SEC,
        {"tot_IB":tot_IB,"tot_IA":tot_IA,"tot_IP":tot_IP,"coal_ex":coal_ex,
         "elec":elec,"clk":clk,"epsP":epsP,"epsP_stream":epsP_stream,
         "spec_IB":spec_IB,"spec_IB_MJkg":spec_IB_MJkg,"m_clinker":m_clinker,
         "fuel_ex_fixed":fuel_ex_fixed,"m_coal":m_coal,
         "I_pb":I_plant_boundary,
...
def write_workbook(openpyxl, SR, SEC, P):
    ...
    for kv in [("Sum section I, Route B [MW]",P["tot_IB"]/1e6),
               ...
               ("Clinker mass flow [kg/s]",P.get("m_clinker",0.0)),
               ("Specific irreversibility [MJ/kg clinker]",P.get("spec_IB_MJkg",float("nan"))),
               ("Specific irreversibility [MW/MW clinker]",P.get("spec_IB",float("nan"))),
               ("Coal chemical exergy, stream [MW]",P["coal_ex"]/1e6),
               ("Coal mass firing [kg/s]",P.get("m_coal",0.0)),
               ("Fuel exergy, fixed basis [MW]",P.get("fuel_ex_fixed",0.0)/1e6),
               ("Electrical work in [MW]",P["elec"]/1e6),
               ("Clinker product exergy [MW]",P["clk"]/1e6),
               ("Overall plant efficiency, fixed fuel basis [%]",P["epsP"]*100),
               ("Overall plant efficiency, stream basis [%]",P.get("epsP_stream",0.0)*100)]:
        ws3.append([kv[0],round(kv[1],4)])
